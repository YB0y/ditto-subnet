...
export function createBackroomMcpServer(props: McpGrantProps) {
  ...
  registerTool(
    'get_screening_failure_diagnostic',
    {
      ...
      description:
        'Read the bounded, sanitized private failure detail and log tail retained for one exact screening attempt, plus court_diagnostic when a held source review stopped inside the automated court. Supply both the agent UUID and attempt UUID; Platform verifies that the attempt belongs to that submission and returns the artifact SHA-256, attempt status, policy version, reason code, and timestamps alongside the diagnostic. court_diagnostic is structured metadata only: error class, escalation code, timeout stage, provider HTTP status, elapsed milliseconds, prompt and completion token counts, and whether a final tool call was returned. It contains no source, prompt, credential, or model text. It is null when the attempt has no court trace, including attempts screened before the field existed. This is operator infrastructure evidence, not a policy verdict, and reading it does not clear, reject, or rescreen the hold. Requires backroom:artifact:read because the private failure text can contain miner-influenced build or runtime diagnostics.',
